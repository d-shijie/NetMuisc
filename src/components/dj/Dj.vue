    ...
    <div class="detail">
      ...
      <div class="info">
        ...
        <div class="nickname">
          ...
          <span>{{djInfo.creator}}</span>
        </div>
        <div class="choices">
          <el-button type="danger" class="el-icon-video-play" size="medium" round>播放全部</el-button>
          <el-button size="medium" class="el-icon-folder-add" round>订阅({{djInfo.subCount}})</el-button>
          <el-button size="medium" class="el-icon-position" round>分享({{djInfo.shareCount}})</el-button>
        </div>
        <div class="desc">
...
export default {
  ...
  methods:{
    getDjDetail(){
      getDjDetail(this.$route.params.id).then(res=>{
