  ...
  <div class="music-list">
    <div class="detail">
      ...
      <div class="info">
        ...
        <div class="creator">
          <span><img :src="musicListDetail.creatorUrl" alt="">{{musicListDetail.creator}}</span>
        <span>{{musicListDetail.createTime|dateFormat}}创建</span>
        </div>
        <div class="choices">
          <el-button type="danger" class="el-icon-video-play" size="medium" round>播放全部</el-button>
          <el-button size="medium" class="el-icon-folder-add" round>收藏{{'('+subCount+')'}}</el-button>
          <el-button size="medium" class="el-icon-position" round>分享{{'('+musicListDetail.shareCount+')'}}</el-button>
          <el-button size="medium"  class="el-icon-download"  round>VIP下载</el-button>
        </div>
        <div class="tags">
          标签:{{musicListDetail.tags.join("/")}}
        </div>
        <div class="size">
...
export default {
  ...
  computed:{
    ...
    desc(){
      return this.musicListDetail.desc.length>50? "太多了......" :this.musicListDetail.desc
    }
  },
  created() {
