<template>
  <div class="personal-fm">
    <div class="left">
      <div class="cover">
        <span v-if="isShowPlay===true" @click="play"><img class="play" src="../../assets/img/play/播放.png" alt=""></span>
        <span v-else @click="pause"><img class="el-icon-video-pause" src="../../assets/img/play/暂停.png" alt=""></span>
      ...
      <div class="btns">
        ...
        <el-button @click="next" class="el-icon-caret-right" circle> </el-button>
    ...
    <div class="right">
      <div class="info">
        <h2>{{fmInfo.name}}</h2>
        <div class="detail">
          <span style="color: #575757;font-size: 12px">专辑:{{fmInfo.album}}</span>
          <span style="color: #575757;font-size: 12px">歌手:{{fmInfo.singer}}</span>
        </div>
        <div class="box">
          <div ref="box" class="lyric">
            <div :data-index='index' ref="lyric"   :class="{active: lyricIndex === index}"  class="text" v-for="(item,index) in lyricArr">
              {{item.lyric}}
            </div>
          </div>
        </div>
...
export default {
    ...
    this.$bus.$on("timeUpdate",(time)=>{
      for (let i = 0; i < this.lyricArr.length; i++) {
        if (time > (parseInt(this.lyricArr[i].time))) {
          ...
          if (i === parseInt(index)) {
            this.lyricIndex = i
            this.$refs.box.style.transform = `translateY(${170 - (30 * (i + 1))}px)`
          }
        }
      }
    })
  ...
  methods:{
    ...
    play(){
      this.$http.get("/song/url",{params:{id:this.fmInfo.id}}).then(res=>{
