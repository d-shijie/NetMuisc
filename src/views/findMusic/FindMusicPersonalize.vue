<template>
  <div class="personalize">
    ...
    <div class="recommend-music-list">
      ...
      <div @mouseenter="mouseEnter(index)" @mouseout="mouseOut" @click="itemClick(item)" class="item" v-for="(item,index) in recommendMusicList" :key=index>
       <div class="img">
         <span>
           <img v-if="currentIndex===index" class="play" src="../../assets/img/play/播放.png" alt="">
         </span>
    ...
    <div class="unique">
      <div @mouseenter="currentIndex1=index" @mouseout="currentIndex1=''" @click="uniqueClick(item)" class="unique-item" v-for="(item,index) in unique">
        <div class="unique-cover">
          <img v-if="currentIndex1===index" class="play2" src="../../assets/img/play/播放.png" alt="">
       ...
       <div @click="newMusicItemClick(item)" class="new-music-item-img">
        ...
        <div class="new-music-item-info">
          ...
          <div class="new-music-item-detail">
            <span style="margin-right: 5px" v-if="item.song.mvid!==0"><MVButton :mvid="item.song.mvid"></MVButton></span>
            <span style="font-size: 12px;color: #8e8e8e">{{item.song.artists[0].name}}</span>
          </div>
        </div>
      </div>
    </div>
    <SmallTitle :text="'推荐MV'"></SmallTitle>
     <div class="recommend-mv">
        <div @click="$router.push('/mv/'+item.id)" class="recommend-mv-item" v-for="(item,index) in recommendMV">
    ...
    <div class="recommend-djs">
      <div @click="$router.push('/dj/'+item.id)" class="recommend-djs-item" v-for="(item,index) in recommendDjs">
        <img :src="item.picUrl" alt="">
        <p>{{item.name}}</p>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    getNotLoginFindMusicPersonalizeMusicList(){
      getNotLoginFindMusicPersonalizeMusicList(9).then(res=>{
    ...
    getFindMusicNewMusic(){
      getFindMusicNewMusic(12).then(res=>{
    ...
    newMusicItemClick(item){
      this.$http.get('/song/url',{params:{id:item.id}}).then(res=>{
        console.log(res)
