...
export default {
  ...
  created() {
    ...
    getDjCategoryRecommend(2001).then(res=>{
      this.create=res.data.djRadios.splice(1,5)
    }).catch(err=>{
      console.log(err)
    })
    getDjCategoryRecommend(3).then(res=>{
      this.emotion=res.data.djRadios.splice(1,5)
    }).catch(err=>{
      console.log(err)
    })
    getDjCategoryRecommend(8).then(res=>{
      this.talkShow=res.data.djRadios.splice(1,5)
    }).catch(err=>{
      console.log(err)
    })
    getDjCategoryRecommend(2).then(res=>{
      this.musicRecommend=res.data.djRadios.splice(1,5)
    }).catch(err=>{
      console.log(err)
    })
    getDjCategoryRecommend(10001).then(res=>{
